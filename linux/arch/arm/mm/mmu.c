...
spinlock_t ro_mytee_pages_lock = __SPIN_LOCK_UNLOCKED();
char ro_pages_stat[RO_PAGES] = {0};
unsigned ro_alloc_last = 0;
int mytee_ro_mapped = 0;


void *mytee_ro_alloc(void)
{
        unsigned long flags;
        int i, j;
        void * alloc_addr = NULL;

        spin_lock_irqsave(&ro_mytee_pages_lock,flags);
        ...
        for (i = 0, j = ro_alloc_last; i < (RO_PAGES) ; i++) {
                ...
                if (!ro_pages_stat[j]) {
                        ro_pages_stat[j] = 1;
                        ro_alloc_last = j+1;
                        alloc_addr = (void*) ((u32)MYTEE_RBUF_VA +  (j << PAGE_SHIFT));
                        break;
                }
        }
        spin_unlock_irqrestore(&ro_mytee_pages_lock,flags);

        return alloc_addr;
}

void mytee_ro_free(void *free_addr)
{
        int i;
        unsigned long flags;

        i =  ((u32)free_addr - (u32)MYTEE_RBUF_VA) >> PAGE_SHIFT;
        spin_lock_irqsave(&ro_mytee_pages_lock,flags);
        ro_pages_stat[i] = 0;
        ro_alloc_last = i;
        spin_unlock_irqrestore(&ro_mytee_pages_lock,flags);
...
unsigned int is_mytee_ro_page(u32 addr)
{
        if( (addr >= (u32)MYTEE_RBUF_VA)
                && (addr < (u32)(MYTEE_RBUF_VA+ MYTEE_ROBUF_SIZE)))
                return 1;
        else return 0;
}
				...
				void *(*alloc)(unsigned long sz))
{
	if (pmd_none(*pmd)) {
		...
		if(mytee_init_flag){
			pte = mytee_ro_alloc();
			if(!pte)
				pte = alloc(PTE_HWTABLE_OFF + PTE_HWTABLE_SIZE);
		}
		else
			pte = alloc(PTE_HWTABLE_OFF + PTE_HWTABLE_SIZE);
#else
			...
			const struct mem_type *type, bool ng)
{
	...
	do {
#ifdef CONFIG_MYTEE
		mytee_ro_memset(EMUL_RO_MEMSET, pmd, __pmd(phys | type->prot_sect | (ng ? PMD_SECT_nG : 0)), 8, 0, 0);
#else
