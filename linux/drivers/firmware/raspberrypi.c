 ...
 */
#ifdef CONFIG_MYTEE
void setBuf(u32 * buf, size_t size, u32 req, void *data, size_t tag_size, u32 pad){
	buf[0] = size;
        buf[1] = req;
        memcpy(&buf[2], data, tag_size); //TODO: validate the memcpy
        buf[size / 4 - 1] = pad;
	asm volatile("mcr p15, 0, %0, c7, c10, 1"::"r"(buf)); // dcache clean
	asm volatile("mcr p15, 0, %0, c7, c10, 1"::"r"(buf+16));
}
			       ...
			       void *data, size_t tag_size)
{
	...
	buf = MYTEE_HYP_SECURE_MAIL_BOX_MSG_VIRT;
	bus_addr = (int) buf + BCM2835_VIRT_TO_BUS_OFFSET;
	...
	setBuf(buf, size, RPI_FIRMWARE_STATUS_REQUEST, data, tag_size, RPI_FIRMWARE_PROPERTY_END);
	...
	memcpy(data, &buf[2], tag_size); //TODO: validate the memcpy
	
	bool is_fail = (buf[1] != RPI_FIRMWARE_STATUS_SUCCESS);
	mytee_down_priv(MYTEE_DOWN_PRIV,0);
