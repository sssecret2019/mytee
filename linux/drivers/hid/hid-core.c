		...
		__u8 *data, int interrupt)
{
	...
	for (n = 0; n < count; n++) {		// if non-secure kbd mode, extract field and process event
		value[n] = min < 0 ?
...
int is_input_code2 = 0;
static void mytee_hid_input_field(struct hid_device *hid, struct hid_field *field,
		__u8 *data, int interrupt)
{
	unsigned n;
	__u8 *temp_data;
	...
	u32* secure_kbd_flag = MYTEE_TRUSTED_KEYBOARD_FLAG_VIRT;
	u32* secure_buffer = MYTEE_HYP_SECURE_HID_KEYBOARD_INBUF_VIRT;
	if(*secure_kbd_flag==0x1){			
#ifdef CONFIG_MYTEE

		if(field->usage->type == EV_KEY){                       // Check if the hid event is regarding a keyboard
			is_input_code1 = 1;                             //      [first field->type == EV_KEY && second field->type == EV_SYN]
			// The first field describes the modifier key status    
		}
		else if(is_input_code1==1 && field->usage->type == EV_SYN){                     // The second field describes the input key code 
			temp_data = (__u8 *)FAKE_DATA;                                                  // [data = usbuid->inbuf = secure buffer] swicthes to the fake_buffer
			if(*secure_buffer!=0 && *secure_buffer!=0x280000)                               // press key
				*(u32*)temp_data = 0x550000;                                            // send asterisk key value to the REE
			else if(*secure_buffer==0x280000)                                               // enter key is pressed
				*(u32*)temp_data = 0x280000;
			else if(*secure_buffer==0x0)
				*(u32*)temp_data = 0;
		}
#endif


		//printk("field->usage->type == EV_KEY : %x", field->usage->type);
		if(field->usage->type == EV_KEY)			// Check if the hid event is regarding akeyboard
			is_input_code2 = 1;			// The first field describes the modifier keys status
		else if(is_input_code2==1 && field->usage->type == EV_SYN){ // The second field describes the input key code
			...
			if(usb_kbd_keycode[*value]!=0){
				keyLoc = usb_kbd_keycode[*value];
				u32 *buf_index = (u32 *) MYTEE_HYP_SECURE_KEYBOARD_BUF_INDEX_VIRT;
				__u8 * event_buf = (u8 *) MYTEE_HYP_SECURE_KEYBOARD_BUF_VIRT;
				if(*buf_index == 4096)	// Circular buffer, can be enhanced
					*buf_index = 0;
				memcpy(event_buf+(*buf_index), &keyLoc, 1);
				*buf_index = ++*buf_index;
			}
			else
				keyLoc = 0;
...
EXPORT_SYMBOL_GPL(mytee_hid_input_field);
#endif
		...
		int interrupt)
{
	...
	mytee_up_priv(MYTEE_UP_PRIV,0,0,0);
        u32* secure_kbd_flag = MYTEE_TRUSTED_KEYBOARD_FLAG_VIRT;
        u32* keyboard_devnum = MYTEE_HYP_SECURE_HID_KEYBOARD_DEVNUM_SAVE_VIRT;
        if(*secure_kbd_flag==0x1 && dev->devnum == *keyboard_devnum){
		data = MYTEE_HYP_SECURE_HID_KEYBOARD_INBUF_VIRT;
	}
	...
	if (csize < rsize) {
		...
		u32* keyboard_devnum = MYTEE_HYP_SECURE_HID_KEYBOARD_DEVNUM_SAVE_VIRT;
		if(*secure_kbd_flag==0x1 && dev->devnum == *keyboard_devnum){
			cdata = (u8 *)MYTEE_HYP_SECURE_HID_KEYBOARD_INBUF_VIRT;
	...
	if (hid->claimed != HID_CLAIMED_HIDRAW && report->maxfield) {
		...
		for (a = 0; a < report->maxfield; a++){
			...
			if(*secure_kbd_flag==0x1 && dev->devnum == *keyboard_devnum){
				cdata = (u8 *)MYTEE_HYP_SECURE_HID_KEYBOARD_INBUF_VIRT;
				mytee_hid_input_field(hid, report->field[a], cdata, interrupt);
				cdata = (u8 *)FAKE_DATA;
			}
			mytee_down_priv(MYTEE_DOWN_PRIV,0);
